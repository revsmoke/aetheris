...
export class SceneEngine {
    ...
    async loadSite(lat, lng) {
        ...
        try {
            ...
            const renderer = new TilesRenderer(tilesUrl);

            // Add API key to tile requests (but not to blob URLs for textures)
            renderer.preprocessURL = (uri) => {
            ...
            renderer.setResolutionFromRenderer(this.renderer);

            // Assign to this.tiles ONLY after setting camera and resolution
            this.tiles = renderer;

            // Set initial resolution immediately
            this.onResize();
    ...
    onResize() {
        ...
        if (this.tiles) {
            // Set resolution explicitly to avoid LOD calculation warnings
            this.tiles.setResolution(width, height);
    ...
    animate() {
        ...
        if (this.tiles) {
            ...
            // Only update if we have valid dimensions to avoid "resolution not set" warnings
            if (width > 0 && height > 0) {
                this.camera.updateMatrixWorld();

                // Re-assert camera and resolution
